...
import LandingMonthlyCourses from "../components/LandingMonthlyCourses";
...
export default async function LandingPage({ searchParams }: LandingPageProps) {
  ...
  const enrolledCourseIdByRegularId = Object.fromEntries(enrolledByCourse.entries());
  const monthlyCards = displayCourses.map((c) => ({
    id: c.id,
    name: c.name,
    price_cents: c.price_cents,
    discount_percent: (c as { discount_percent?: number | null }).discount_percent ?? null,
    promotion_tiers: (c as { promotion_tiers?: unknown }).promotion_tiers ?? null,
    active_enrollment_count: (c as { active_enrollment_count?: number }).active_enrollment_count ?? 0,
    registration_open_at: c.registration_open_at,
    registration_close_at: c.registration_close_at,
    course_start_at: c.course_start_at,
    course_end_at: c.course_end_at,
    base_course: (c.base_course as { name?: string; summary?: string } | null) ?? null,
    program: (c.program as { name?: string } | null) ?? null,
  }));
      ...
      <section className="relative z-10 border-t border-white/10 bg-white/[0.02] px-4 py-16 sm:px-6 sm:py-20 lg:py-24">
        <div className="mx-auto max-w-[var(--container-max)]">
          ...
          {displayCourses.length > 0 ? (
            <LandingMonthlyCourses
              courses={monthlyCards}
              enrolledCourseIdByRegularId={enrolledCourseIdByRegularId}
            />
          ) : (
